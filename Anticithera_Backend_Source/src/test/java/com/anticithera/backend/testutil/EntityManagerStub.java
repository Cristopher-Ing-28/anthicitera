...
public class EntityManagerStub implements EntityManager {
    ...
    public <T> T find(Class<T> entityClass, Object primaryKey) {
        if (entities.containsKey(entityClass)) {
            return entityClass.cast(entities.get(entityClass).get(primaryKey));
        }
        return null;
    }

    public void addEntity(Object primaryKey, Object entity) {
        entities.computeIfAbsent(entity.getClass(), k -> new HashMap<>()).put(primaryKey, entity);
    }

    @Override
    public <T> TypedQuery<T> createQuery(String qlString, Class<T> resultClass) {
        return new TypedQueryStub<>(singleResult, resultList, exceptionToThrow);
    }
    ...
    // Unimplemented methods of EntityManager interface
    ...
    @Override public <T> T find(Class<T> entityClass, Object primaryKey, LockModeType lockMode, Map<String, Object> properties) { return null; }
